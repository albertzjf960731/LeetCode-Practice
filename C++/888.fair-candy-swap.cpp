...
public:
    vector<int> fairCandySwap(vector<int>& A, vector<int>& B) {
        int sums_a = accumulate(A.begin(), A.end(), 0);
        int sums_b = accumulate(B.begin(), B.end(), 0);
        int diff = (sums_a-sums_b) / 2;
        
        unordered_set<int> hash_set(B.begin(), B.end());
        for (int num: A) {
            if (hash_set.count(num-diff)) {
