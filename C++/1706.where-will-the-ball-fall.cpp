...
public:
    vector<int> findBall(vector<vector<int>>& grid) {
     vector<int> ans;
        for (int j=0; j<grid[0].size(); ++j) ans.push_back(solve(grid, j));
        return ans;
    }
    
    int solve(vector<vector<int>>& grid, int col) {        
        for (int i=0; i<grid.size(); ++i) {
            if (col==0 && grid[i][col]==-1) return -1;
            if (col==grid[0].size()-1 && grid[i][col]==1) return -1;
            if (grid[i][col]==1 && grid[i][col+1]==-1) return -1;
            if (grid[i][col]==-1 && grid[i][col-1]==1) return -1;
