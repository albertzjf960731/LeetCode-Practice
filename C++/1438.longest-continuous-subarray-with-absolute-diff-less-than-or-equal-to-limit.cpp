...
public:
    int longestSubarray(vector<int>& nums, int limit) {
        ...
        for (int i=0, j=0; j < nums.size(); ++j) {
            while (!qmax.empty() && nums[j]>qmax.back()) 
                qmax.pop_back();
            qmax.push_back(nums[j]);

            while (!qmin.empty() && nums[j]<qmin.back()) 
                qmin.pop_back();
            qmin.push_back(nums[j]);
            
            if (qmax.front() - qmin.front() > limit) {
                if (qmax.front()==nums[i]) qmax.pop_front();
                if (qmin.front()==nums[i]) qmin.pop_front();
                ++i;
            }
            ans = max(ans, j-i+1);
        }
