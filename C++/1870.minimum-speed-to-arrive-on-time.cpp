...
public:
    int minSpeedOnTime(vector<int>& dist, double hour) {
        ...
        while (left < right) {
            ...
            double cnt = 0;
            for (int i=0; i<dist.size()-1; ++i) cnt += (dist[i] - 1) / mid + 1;
            cnt += dist.back() * 1.0 / mid;
            
            if (cnt > hour) left = mid + 1;
            else right = mid;
        }
        double cnt = 0;
        for (int i=0; i<dist.size()-1; ++i) cnt += (dist[i] - 1) / left + 1;
        cnt += dist.back() * 1.0 / left;
        if (cnt <= hour) return left;
        else return -1;
    }
};
// @lc code=end
