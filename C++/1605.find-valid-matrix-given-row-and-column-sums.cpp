...
public:
    vector<vector<int>> restoreMatrix(vector<int>& rowSum, vector<int>& colSum) {
        ...
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> row_pq, col_pq;
        for (int i=0; i<n; ++i) row_pq.push(make_pair(rowSum[i], i));
        for (int j=0; j<m; ++j) col_pq.push(make_pair(colSum[j], j));

        while (!row_pq.empty() && !col_pq.empty()) {
            auto [rs, r] = row_pq.top(); row_pq.pop();
            auto [cs, c] = col_pq.top(); col_pq.pop();

            int num = min(rs, cs);
            ans[r][c] = num;

            rs -= num; 
            if (rs != 0) row_pq.push({rs, r});
            cs -= num;
            if (cs != 0) col_pq.push({cs, c});
        }
        return ans;
