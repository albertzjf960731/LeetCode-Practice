...
public:
    int distributeCookies(vector<int>& cookies, int k) {
        vector<int> sums(k, 0);
        int ans = INT_MAX;
        DFS(nums, 0, k, sums, ans);
        return ans;
    }

    void DFS(vector<int>& nums, int idx, int k, vector<int>& sums, int& ans) {
        if (idx == nums.size()) {
            int maxNum = *max_element(sums.begin(), sums.end());
            ans = min(ans, maxNum);
            return;
        }
