...
public:
    ...
    bool insert(int val) {
        hash_map[val].push_back(nums.size());
        nums.push_back(make_pair(val, hash_map[val].size()-1));
    ...
    bool remove(int val) {
        ...
        if (it != hash_map.end()){
            auto last = nums.back(); 
            int size = hash_map[val].back();
            
            hash_map[last.first][last.second] = hash_map[val].back();
            // hash_map[last.].pop_back();
            // hash_map[last].push_back(size);

            nums[size] = last;
            nums.pop_back();

            if(hash_map[val].size()>1)
                hash_map[val].pop_back();
            else
                hash_map.erase(val);
...
int main() {
    RandomizedCollection* obj = new RandomizedCollection();
    obj->insert(10); obj->insert(10); obj->insert(20); obj->insert(20); obj->insert(30); obj->insert(30); 
    obj->remove(10);
    obj->remove(10);
    return 0;
}